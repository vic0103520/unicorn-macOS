...
After confirmation, the installer removes quarantine from the already validated candidate. It acquires an installation lock, stages a copy in the destination directory, validates the staged copy, and preserves a valid existing `unicorn.app` as a backup before the final same-filesystem move. It snapshots the complete set of processes executing the preserved executable, validates the installed copy, checks that quarantine is absent, and requires both a successful Launch Services registration command and a matching path in the registration database. Immediately before each `SIGTERM`, it revalidates that the snapshotted PID still maps the preserved executable, then waits up to five seconds for every snapshotted mapping to disappear. Inspection ambiguity or a genuine signaling failure aborts the replacement, and the installer never escalates to `SIGKILL`. The fixed grace period allows orderly shutdown; the shorter attempt-count override is reserved for deterministic repository tests. macOS shell process interfaces do not provide a PID-bound signaling handle, so an unavoidable residual PID-reuse race remains between the final `lsof` identity check and `kill`; the adjacent revalidation minimizes but cannot eliminate the possibility of signaling a process that acquires the same PID in that interval.
