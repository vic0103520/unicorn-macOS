...
After confirmation, the installer removes quarantine from the already validated candidate. It acquires an installation lock, stages a copy in the destination directory, validates the staged copy, and preserves a valid existing `unicorn.app` as a backup before the final same-filesystem move. It validates the installed copy, checks that quarantine is absent, requires both a successful Launch Services registration command and a matching path in the registration database, and then terminates the installed Unicorn process if one is running.
