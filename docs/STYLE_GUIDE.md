...
## Core Philosophy
The system architecture is defined in the [specification](SPECIFICATION.md#architecture). This guide defines the Swift coding practices used to preserve its pure-core and framework-shell boundaries.
...
The [canonical three-tier definition](SPECIFICATION.md#architecture) owns layer responsibilities. Code must preserve those boundaries: domain and presentation computations remain deterministic, while framework effects remain in the `InputMethodKit` shell.
